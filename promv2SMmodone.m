function [f,f_ko,v,v_ko,status1,lostxns,probtfgene] =  promv2SMmodone(model,expression,expressionid,regulator,targets,litevidence,prob_prior,subsets,v11,v12,KAPPA,datathresh,probtfgene,sizeflag)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
...
% f0 = -f_wt;
solntmp = optimizeCbModelSM(modeltmp,[],'one');
v = solntmp.x;
f0 = solntmp.f;
...
%     [v00(ci,:),f00(ci),status1(ci)] = glpk(-weights11,A,dxdt0,lb11,ub11,ctype1,[],[],param);
    modsolntmp = optimizeCbModelSM(modelmodtmp,[],'one');
    if ~isempty(modsolntmp.x)
        v00(ci,:) = modsolntmp.x';
    else
...
%         [v00(ci,:),f00(ci),status1(ci)] = glpk(-weights11,A,dxdt0,lb11,ub11,ctype1,[],[],param);
        ...
        if (coun > 50),
            ...
            % the problem at the iteration number - ci;
                ...
                modeltmp2.stoic = S;
                solntmp2 = optimizeCbModelSM(modeltmp2,[],'one');
%             ct2 = ctype;
%             ct2(ct2 == 'L') = 'U';
%             ct2(ct2 == 'G') = 'L';
%             ct2(ct2 == 'E') = 'S';
...
%             [v30,f30,status30] = glpk(-weights00,A,dxdt0,lb11,ub11,ctype1);
            %if abs((f3- v30(find(weights)))/abs(f3)) < 0.1
                f00(ci) = solntmp2.f;
                v00(ci,find(weights)) = solntmp2.f;
%             f00(ci) = -f3;
