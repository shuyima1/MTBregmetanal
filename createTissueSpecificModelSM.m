function [tissueModel,Rxns] = createTissueSpecificModelSM(model,expressionData,proceedExp,orphan,exRxnRemove,solver,options,funcModel)
...
parsedGPRix = cellfun(@(x) strmatch(x,model.genes,'exact'),parsedGPR,'UniformOutput',false);
if proceedExp == 0
...
%[ExpressedRxns,UnExpressedRxns,unknown] = mapProbes(parsedGPR,corrRxn,locus,genePresence);
 [ExpressedRxns,UnExpressedRxns,unknown] = mapProbes(parsedGPRix,corrRxn,locus,genePresence);
