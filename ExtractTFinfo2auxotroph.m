for i = 1:size(uTF,1);
    ...
    eval([TFname 'Pints = probtfgeneB7_aa(x);'])
    ...
    if flag
        ...
        for j = 1:size(blah,2)
            tmp = find(blah(:,j) > 0);
            eval([TFname 'RxnInfo{j,1} = Beste7H9aa.rxns(tmp);'])
            eval([TFname 'RxnInfo{j,2} = Beste7H9aa.grRules(tmp);'])
        end
        eval([TFname 'RxnInfo(:,3) = Beste7H9aa.genes(' TFname 'affectedix(:,1));'])
        eval([TFname 'RxnInfo(:,4) = num2cell(' TFname 'affectedix(:,2));'])
        eval(['tmpix = cellfun(@(x) strmatch(x,Beste7H9aa.genes),' TFname 'RxnInfo(:,3));'])
        eval([TFname 'RxnInfo(:,5) = num2cell(grRatioB7aa(tmpix));'])
    ...
    eval([TFname 'intInfo = [' TFname 'Mints num2cell([' TFname 'PMints grRatioB7aa(ib)]) GriffinData(GrifmetIX,7:8)];'])
    eval([TFname 'intInfo = [{''Regulator'' ''Target'' ''ProbTF'' ''grRatioKO'' ''GriffinP'' ''Sassetti''};' TFname 'intInfo];'])

    eval(['save ' TFname 'Analysis_7H9aa ' TFname '*'])
    
    fid = fopen([TFname 'intInfo_7H9aa.txt'],'w');
