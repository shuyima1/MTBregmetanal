function [f,f_ko,v,v_ko,status1,lostxns,probtfgene] =  promv2SMmod(model,expression,expressionid,regulator,targets,litevidence,prob_prior,subsets,v11,v12,KAPPA,datathresh,probtfgene,sizeflag)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
...
%     [v00(ci,:),f00(ci),status1(ci)] = glpk(-weights11,A,dxdt0,lb11,ub11,ctype1,[],[],param);
    modsolntmp = optimizeCbModelSM(modelmodtmp);
    v00(ci,:) = modsolntmp.x';
    f00(ci) = modsolntmp.f;
...
%     [v2(ci,:),f1(ci),status] = glpk(-weights,S,dxdt,lbg,ubg,ctype);
    solntmp2 = optimizeCbModelSM(modeltmp2);
