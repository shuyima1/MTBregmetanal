for i = 1:size(uTF,1);
    ...
    if flag
        eval(['blah = iNJ661m.rxnGeneMat(:,' TFname 'affectedix(:,1));'])
        eval([TFname 'RxnInfo = cell(size(blah,2),2);'])
        for j = 1:size(blah,2)
            tmp = find(blah(:,j) > 0);
            eval([TFname 'RxnInfo{j,1} = iNJ661m.rxns(tmp);'])
            eval([TFname 'RxnInfo{j,2} = iNJ661m.grRules(tmp);'])
        end
        eval([TFname 'RxnInfo(:,3) = iNJ661m.genes(' TFname 'affectedix(:,1));'])
        eval([TFname 'RxnInfo(:,4) = num2cell(' TFname 'affectedix(:,2));'])
        eval(['tmpix = cellfun(@(x) strmatch(x,iNJ661m.genes),' TFname 'RxnInfo(:,3));'])
        eval([TFname 'RxnInfo(:,5) = num2cell(grRateKO(tmpix));'])
    ...
    eval([TFname 'intInfo = [' TFname 'Mints num2cell([' TFname 'PMints grRateKO(ib)]) GriffinData(GrifmetIX,7:8)];'])
    eval([TFname 'intInfo = [{''Regulator'' ''Target'' ''ProbTF'' ''grRateKO'' ''GriffinP'' ''Sassetti''};' TFname 'intInfo];'])

    eval(['save ' TFname 'Analysis ' TFname '*'])
