function [flux conv] = geometricFBASMprom(model,varargin)
...
if ~isfield(model,'csense')
    ...
    if length(model.csense)~= size(A,1),
...
% determine optimum
FBAsolution = optimizeCbModelSM(model);
% % prm.itlim = 1000000;
% % [v00,f00,status1] = glpk(-model.c,A,b,L,U,csense,[],[],prm);
ind = find(model.c);
if length(ind) == 1
...
%         f = sparse(b2+2*a2,1); f((b2+1):end) = -1;
        f = sparse(b2+2*s2,1); f((b2+1):end) = -1;
        opt = -Z(k);
        
        allA = [allA,sparse(b1,2*s2);
            speye(s2,s2),sparse(s2,b2-s2),-speye(s2),speye(s2);
            f(:)'];                                             %#ok<AGROW>
        allB = [allB;mu(:,k);opt];                              %#ok<AGROW>
        allL = [allL;zeros(2*s2,1)];                            %#ok<AGROW>
        allU = [allU;inf*ones(2*s2,1)];
...
%         allU = [allU;inf*ones(2*a2,1)];                         %#ok<AGROW>
    ...
    [b1,b2] = size(allA);
    f = zeros(b2+2*s2,1); f((b2+1):end) = -1;
    allA = [allA,sparse(b1,2*s2);
        speye(s2,s2),sparse(s2,b2-s2),-speye(s2),speye(s2)];	%#ok<AGROW>
    allB = [allB;M];                                            %#ok<AGROW>
    allL = [allL;zeros(2*s2,1)];                                %#ok<AGROW>
    allU = [allU;inf*ones(2*s2,1)];                             %#ok<AGROW>
