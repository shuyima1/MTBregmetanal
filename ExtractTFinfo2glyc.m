for i = 1:size(uTF,1);
    ...
    eval(['[c ia ib] = intersect(' TFname 'ints(:,2),Beste7H9glyc.genes);'])
    eval([TFname 'Mints = ' TFname 'ints(ia,:);'])

    % Finds the ProbTFs associated with the TF of interest
    eval([TFname 'Pints = probtfgeneB7glyc2e2(x);'])
    ...
    if flag
        ...
        for j = 1:size(blah,2)
            tmp = find(blah(:,j) > 0);
            eval([TFname 'RxnInfo{j,1} = Beste7H9glyc.rxns(tmp);'])
            eval([TFname 'RxnInfo{j,2} = Beste7H9glyc.grRules(tmp);'])
        end
        eval([TFname 'RxnInfo(:,3) = Beste7H9glyc.genes(' TFname 'affectedix(:,1));'])
        eval([TFname 'RxnInfo(:,4) = num2cell(' TFname 'affectedix(:,2));'])
        eval(['tmpix = cellfun(@(x) strmatch(x,Beste7H9glyc.genes),' TFname 'RxnInfo(:,3));'])
        eval([TFname 'RxnInfo(:,5) = num2cell(grRatioB7glyc(tmpix));'])
    ...
    eval([TFname 'intInfo = [' TFname 'Mints num2cell([' TFname 'PMints grRatioB7glyc(ib)]) GriffinData(GrifmetIX,7:8)];'])
    eval([TFname 'intInfo = [{''Regulator'' ''Target'' ''ProbTF'' ''grRatioKO'' ''GriffinP'' ''Sassetti''};' TFname 'intInfo];'])

    eval(['save ' TFname 'Analysis_glyc2 ' TFname '*'])
    
    fid = fopen([TFname 'intInfo_glyc2.txt'],'w');
