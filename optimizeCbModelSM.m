function FBAsolution = optimizeCbModelSM(model,osenseStr, minNorm, allowLoops)
...
% Solve initial LP
if allowLoops
    solution = solveCobraLPSM(LPproblem);
else
...
if strcmp(minNorm, 'one')
    ...
    [nrows,ncols] = size(model.stoic);

    LPproblem2.A = [model.S sparse(nMets,2*ncols);
        speye(ncols,ncols) sparse(ncols,nRxns-ncols) speye(ncols,ncols) sparse(ncols,ncols);
        -speye(ncols,ncols) sparse(ncols,nRxns-ncols) sparse(ncols,ncols) speye(ncols,ncols);
        model.c' sparse(1,2*ncols)];
    LPproblem2.c = [zeros(nRxns,1);ones(2*ncols,1)];
    LPproblem2.lb = [model.lb;zeros(2*ncols,1)];
    LPproblem2.ub = [model.ub;10000*ones(2*ncols,1)];
    LPproblem2.b = [LPproblem.b;zeros(2*ncols,1);solution.obj-(10^-6)];
    if ~isfield(model,'csense')
        % If csense is not declared in the model, assume that all
        % constraints are equalities.
    ...
    LPproblem2.csense((nMets+1):(nMets+2*ncols)) = 'G';
    LPproblem2.csense(nMets+2*ncols+1) = 'G';
    LPproblem2.csense = columnVector(LPproblem2.csense);
    LPproblem2.osense = 1;
    % Re-solve the problem
    if allowLoops
        solution = solveCobraLPSM(LPproblem2); %,printLevel,minNorm);
