function [grRatio,grRateKO,grRateWT,hasEffect,delRxns,fluxSolution] = singleGeneDeletionSM(model,method,geneList,verbFlag)
...
for i = 1:nDelGenes
    ...
    if (hasEffect(i))
        switch method
            ...
            otherwise
                solKO = optimizeCbModel(modelDel,'max','one');
        end
        if (solKO.stat == 1)
            grRateKO(i) = solKO.f;
