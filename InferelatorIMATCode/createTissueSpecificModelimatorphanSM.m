function [tissueModel,Rxns] = createTissueSpecificModelimatorphanSM(model, ...
                                                  expressionData,proceedExp,orphan,exRxnRemove,solver,epsilon,options,funcModel)
...
%         epsilon = 1;
        ...
        if orphan == 1
            ...
            orphans = model.rxns(find(strcmp('',model.grRules)));
...
emptycells = cellfun(@(x) isequal({''},x),parsedGPR(:,1));
parsedGPR(emptycells,:) = [];
corrRxn(emptycells,:) = [];
...
%     loc = strmatch(Transcripts.Data{i},ExpressionData.Transcript,'exact');
    loc = find(strcmp(Transcripts.Data{i},ExpressionData.Transcript));
    for j = 1:length(loc)
        total(j) = sum(ExpressionData.Data(loc(j),:));
        ExpressionData.Locus(loc(j)) = 0;
    end
